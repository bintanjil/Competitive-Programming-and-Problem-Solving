...
void solve()
{
    ...
    /// For v1 (calculating from the left)
    int sum = 0;
    for (int i = 1; i <= n; i++) {
        sum += a[i];
        if (sum >= v) {
    ...
    /// For v2 (calculating from the right)
    sum = 0;
    for (int i = n; i >= 1; i--) {
        sum += a[i];
        if (sum >= v) {
    ...
    /// If the number of partitions is not enough, output -1
    if ((int)v1.size() - 1 < m) {
        cout << "-1" << endl;
        return;
    }
    ...
    ll ans = 0;
    for (int i = 0; i < m + 1; i++) {
        int l = v1[i] + 1, r = v2[m - i] - 1;
        if (l <= r) {
            ll s = pre_sum[r] - pre_sum[l - 1];
            ans = max(ans, s);
        }
    }
