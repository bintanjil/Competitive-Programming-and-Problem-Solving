...
void solve()
{
    ...
    vector<int> cumelative_count(n + 2, 0);
    for (int i = n; i >= 1; i--) {
        cumelative_count[i] = cnt[i] + cumelative_count[i + 1];
    }

    long long answer = 0;
    for (int splitIndex = 1; splitIndex < n; splitIndex++) {
        ll leftCount = cumelative_count[splitIndex];
        ll rightCount = cumelative_count[n - splitIndex];
        ll commonCount = cumelative_count[max(splitIndex, n - splitIndex)];
        answer += (leftCount) * rightCount - commonCount;
    }
