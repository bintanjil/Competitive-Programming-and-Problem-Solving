...
void solve()
{
    ...
    vector<ll> row_sums(n, 0);

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cin >> arrays[i][j];
        }
        sort(arrays[i].begin(), arrays[i].end()); // Sort each row
        for (int j = 1; j < m; j++) {
            arrays[i][j] += arrays[i][j - 1]; // Compute prefix sums
        }
        row_sums[i] = arrays[i].back(); // Store total sum of each array
    }

    sort(row_sums.begin(), row_sums.end()); // Sort the row sums
